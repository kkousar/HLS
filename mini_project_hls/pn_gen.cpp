...
#pragma HLS LOOP_TRIPCOUNT
    	...
    	x1[(n+31)%Nc] = x1[(n+3)%Nc] ^ x1[n%Nc];
    	x2[(n+31)%Nc] = x2[(n+3)%Nc] ^ x2[(n+2)%Nc] ^ x2[(n+1)%Nc] ^ x2[n%Nc];
    	x1[(n+32)%Nc] = x1[(n+4)%Nc] ^ x1[(n+1)%Nc];
    	x2[(n+32)%Nc] = x2[(n+4)%Nc] ^ x2[(n+3)%Nc] ^ x2[(n+2)%Nc] ^ x2[(n+1)%Nc];
    	// Compute the output bits
    	if (n >=Nc) {
    		bool bit1 = (x1[n%Nc] ^ x2[n%Nc]);
    		bool bit2 = (x1[(n+1)%Nc] ^ x2[(n+1)%Nc]);
    		count=count+2;
